...
class ModelTests: XCTestCase {
    ...
    func test_FlickrPhotoSearchResult_decodesValidJSON_successfully() {
        let json = """
        ...
        guard let mappedModel = try? JSONDecoder().decode(FlickrPhotoSearchResult.self, from: json) else { return XCTFail() }
        
        XCTAssertEqual(mappedModel.totalCount, 1926027)
        XCTAssertEqual(mappedModel.photos.count, 1)
    }
    
    func test_FlickrPhoto_decodesValidJSON_successfully() {
        let json = """
        ...
        guard let mappedModel = try? JSONDecoder().decode(FlickrPhoto.self, from: json) else { return XCTFail() }
